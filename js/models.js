...
class StoryList {
  ...
  async addStory(user, newStory) {
    ...
    const config = { token, story: newStory };
    const response = await axios.post(`${BASE_URL}/stories`, config);
...
class User {
  ...
  static async signup(username, password, name) {
    const response = await axios.post(`${BASE_URL}/signup`, { user: { name, username, password } });
  ...
  static async loginViaStoredCredentials(token, username) {
    try {
      ...
      const response = await axios(`${BASE_URL}/users/${username}`, config);
